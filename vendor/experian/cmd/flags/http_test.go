...
func TestHttp_ToHttpRequestsEmpty(t *testing.T) {
	requestFlags := []string{}
...
func TestHttp_String(t *testing.T) {
	...
	assert.Contains(t, result, "post:/ping")
...
func TestHttp_getWarmupHTTPRequestsWithCompression(t *testing.T) {
	h := HTTP{
		Requests:    stringArray{"post:/api:body"},
		Compression: "gzip",
	}
	requests, err := h.getWarmupHTTPRequests()
	require.NoError(t, err)
